# MEMORA Known Limitations & Technical Scope (Phase 1)
...
* **Limitation**: The storage is local-first. Concurrent writes across multiple distributed processes require file-locking or a dedicated service tier. For hackathon Phase 1, single-instance FastAPI operation is the intended architecture.
...
* **Current Constraint**: Free-tier accounts in `sibyl-memory-client` are subject to a local 5 MB cap (`CapExceededError`). The test suite and Phase 1 demonstrations use light metadata that remains well below this threshold.
