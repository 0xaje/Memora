...
class MemoryWriter:
    ...
    def write_incident(self, incident: IncidentMemory) -> Dict[str, Any]:
        """
    ...
    def write_decision(self, decision: DecisionMemory) -> Dict[str, Any]:
        """
        ...
        logger.info("Writing decision to Sibyl: name=%s, incident=%s, final_risk=%s",
                    name, decision.incident_id, decision.final_risk)
        try:
            entity_row = client.set_entity(
                category=category,
                name=name,
    ...
    def write_outcome(self, outcome: OutcomeMemory) -> Dict[str, Any]:
        """
        ...
        logger.info("Writing outcome to Sibyl: name=%s, incident=%s, status=%s",
                    name, outcome.incident_id, status)
        try:
            entity_row = client.set_entity(
                category=category,
                name=name,
    ...
    def write_unresolved_risk(self, risk: UnresolvedRiskMemory) -> Dict[str, Any]:
        """
        ...
        logger.info("Writing unresolved risk to Sibyl: name=%s, location=%s, severity=%s",
                    name, risk.location, risk.severity)
        try:
    ...
    def write_operational_lesson(self, lesson: OperationalLesson) -> Dict[str, Any]:
        """
        ...
        logger.info("Writing operational lesson to Sibyl: name=%s, location=%s, rule=%s",
                    name, lesson.location, lesson.rule_or_insight)
        try:
            return client.set_entity(
                category=category,
                name=name,
