...
class OperationalLesson(BaseModel):
    ...
    successful_mitigation: Optional[str] = Field(None, description="Action confirmed to resolve or mitigate the threat")
    escalation_recommendation: Optional[str] = Field(None, description="Recommended escalation if pattern repeats")
    recurrence_count: int = Field(default=1, description="Number of times this operational pattern has occurred")
